...
var PrintControl = function(){
    ...
    self.cmdArray = [{},
        ...
        {id:4, code:'P04', fromType:prop.digestFromType.FIX, des:"改变算奖队列不再算奖"},
...
PrintControl.prototype.handleP04 = function(user, headNode, bodyNode, cb)
{
	...
	async.waterfall([
		function(cb){
			...
			//var cond = {orderId:bodyNode.outerId};
			var conn = dc.main.getConn();
			var sql = "select id,termCode,outerId from tticket where orderId = (select id from torder where outerId = '"+ bodyNode.outerId + "')";
			log.info(sql);
			conn.execute(sql, [],function(err, data){
			//ticketTable.find(cond,{id:1,termCode:1,gameCode:1}).sort({id:1}).toArray(function(err, data){
				if(err){
				   log.error(err);
				   cb(err);
				}else{
				 	async.eachSeries(data, function(ticket,callback){
    	                ...
    	                async.eachSeries(seq , function(seqId, call){
							...
							table.findAndRemove({"_id":ticket.id},{},function(err){
								call(err);
							});
						},function(err){
							callback(null);
