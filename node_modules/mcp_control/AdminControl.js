...
AdminControl.prototype.handleAD21 = function (user, headNode, bodyNode, cb) {
    ...
    if(bodyNode.data.printStatus == ticketPrintStatus.PRINT_SUCCESS){
        ...
        table.update(cond, obj, [], function (err, data) {
            if (err) {
                log.info(err);
                cb(ec.E0999);
            }
...
AdminControl.prototype.handleAD23 = function (user, headNode, bodyNode, cb) {
    ...
    if(cmd == 1000){ //全部重新出票
        ...
        table.update(bodyNode.cond,{$set:{printQueenStatus: ticketPrintQueenStatus.WAITING_GET}}, {multi:true}, function(){
            cb(null, backBodyNode);
        })
    }else if (cmd == 1510){//单个失败
        table.findAndRemove(bodyNode.cond, {}, [], function(err, ticket){
            if(err){
                cb(err);
            }else{
                if(ticket){
