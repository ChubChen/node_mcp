...
var AdminControl = function () {
    ...
    self.cmd = {'AD01': 1, 'AD02': 2, 'AD03': 3, 'AD04': 4,
        ...
        'AD18': 18, 'AD19': 19, 'AD20': 20, 'AD21': 21 , 'AD22': 22, 'AD23': 23, 'AD24': 24};
    self.cmdArray = [
        {},
        ...
        {id: 21, code: 'AD21', fromType: prop.digestFromType.CACHE, des: "返回出票状态"},
        {id: 22, code: 'AD22', fromType: prop.digestFromType.CACHE, des: "增加游戏可售"},
        {id: 23, code: 'AD23', fromType: prop.digestFromType.CACHE, des: "处理出票队列"},
        {id: 24, code: 'AD24', fromType: prop.digestFromType.CACHE, des: "重新通知票状态"}
    ];
};
...
AdminControl.prototype.checkAD22 = function (user, headNode, bodyNode, cb) {
    cb(null);
};
AdminControl.prototype.checkAD23 = function (user, headNode, bodyNode, cb) {
    cb(null);
};

AdminControl.prototype.checkAD24 = function (user, headNode, bodyNode, cb) {
    cb(null);
};
...
AdminControl.prototype.handleAD20 = function (user, headNode, bodyNode, cb) {
    ...
    table.update(bodyNode.cond, {$set: {gamePermission: permission}}, [], function (err, data) {
        if (err) {
            ...
            //增加通知
            var game={};
            game.gameCode = bodyNode.cond._id;
...
AdminControl.prototype.handleAD21 = function (user, headNode, bodyNode, cb) {
    ...
    var ticket = bodyNode.data;
    ticket.id = parseInt(bodyNode.data.id, 10);
    if(bodyNode.data.printStatus == ticketPrintStatus.PRINT_SUCCESS){
        ticket.printTime = new Date().getTime();
    ...
    }else if(bodyNode.data.printStatus  == ticketPrintStatus.PRINT_FAILURE) {
        var doc = {$set:{status:ticketPrintQueenStatus.PRINT_FAILURE, printQueenStatus:ticketPrintQueenStatus.PRINT_FAILURE}};
        var sort = {};
        table.findAndModify({"_id":ticket.id}, sort, doc, [], function(err, data){
            ticketSer.printFail(ticket, bodyNode.data.termCode, function(err, backTicket){
                if(err){
                    log.error(err);
                    cb(ec.E2057);
                }else{
        ...
        ticketTable.findOne(cond,{}, [], function (err, ticket) {
            ...
            table.save(pTicket, [], function(err, data){
                delete bodyNode.data.id;
                var obj = {$set:bodyNode.data};
                log.info(obj);
                table.update({_id:ticket.id}, obj, [], function (err, data) {
                    if (err) {
                        log.info(err);
                        cb(ec.E0999);
                    }
 ...
 * @param cb
 */
AdminControl.prototype.handleAD22 = function (user, headNode, bodyNode, cb) {
    var self = this;
    var backBodyNode = {};
    var game = bodyNode.game;
    var table = dc.mg.get("gamePermission");
    table.save(game, [] , function(err){
       cb(err);
    });
};
 ...
 * @param cb
 */
AdminControl.prototype.handleAD24 = function (user, headNode, bodyNode, cb) {
    var self = this;
    var backBodyNode = {};
    var table = dc.main.get("tticket");
    table.findOne({id: bodyNode.id}, {}, [], function(err, ticket){
        if(err){
            cb(err);
        }else{
 ...
 * @param cb
 */
AdminControl.prototype.handleAD23 = function (user, headNode, bodyNode, cb) {
    var self = this;
    var backBodyNode = {};
    var table = dc.mg.get("printqueen");
    var cmd = bodyNode.cmd;
    var cond = {"_id":parseInt(bodyNode.id,10)};
    if(cmd == 1000){ //全部重新出票
        table.update({printQueenStatus:ticketPrintQueenStatus.TAKE_AWAY},{$set:{printQueenStatus: ticketPrintQueenStatus.WAITING_GET}}, {multi:true}, function(){
            cb(null, backBodyNode);
        });
    }else if (cmd == 1500){//全部失败
        var hasNext = true;
        async.whilst(
                ...
                table.findAndRemove({}, {}, [], function(err, ticket){
                    if(err)
                    {
                        ...
                        if(ticket){
                            ...
                            ticketSer.printFail(ticket, ticket.termCode, function(){
                                callback(null);
                            });
                        }else{
                            hasNext = false;
    ...
    }else if (cmd == 1010){//单个重新出票
        table.update(cond,{$set:{printQueenStatus: ticketPrintQueenStatus.WAITING_GET}}, {multi:true}, function(){
            cb(null, backBodyNode);
        })
    }else if (cmd == 1510){//单个失败
         log.info(cond);
        table.findAndRemove(cond, {}, [], function(err, ticket){
            if(err){
                ...
                if(ticket){
                    log.info(ticket);
                    ticket.id = ticket._id;
                    ticketSer.printFail(ticket, ticket.termCode, function(err){
                       cb(err, backBodyNode);
                   })
                }
            }
