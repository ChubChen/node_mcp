...
ChannelQueryControl.prototype.checkCQ22 = function(user, headNode, bodyNode, cb)
{
    ...
    if(bodyNode.pType == undefined || bodyNode.pType == null || bodyNode.pType == ""){
        ...
        if(bodyNode.pType instanceof Array){
            if(bodyNode.gameCode == "T51" && bodyNode.length == 1 && bodyNode[0] == "06"){
                bodyNode.pType = ["01","02","03","04","05"];
            }else if(bodyNode.gameCode= "T52" && bodyNode.length == 1 && bodyNode[0] == "05"){
                bodyNode.pType = ["01","02","03","04"];
            }
            cb(null);
            return;
        }else{
                    ...
                    if(ticketGame.type == gameType.Jingcai ){
                        ...
                        if(bodyNode.pType){
                            for(var iterm in bodyNode.pType){
                                var key = self.pTypeToKey[bodyNode.gameCode.toString()+bodyNode.pType[iterm].toString()];
                                fields[key] = 1;
                            }
                        }
