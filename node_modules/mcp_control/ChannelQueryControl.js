...
var ChannelQueryControl = function(){
    ...
    self.cmd = {'CQ01':{ code:'CQ01', fromType:prop.digestFromType.FIX, des:"期次查詢"},
        ...
        'CQ21':{code:'CQ21', fromType:prop.digestFromType.FIX, des:"查询竞猜赛果"},
    };
...
ChannelQueryControl.prototype.checkCQ21 = function(user, headNode, bodyNode, cb)
{
    ...
    if(bodyNode.matchCode instanceof Array){
        cb(null);
        return;
    }else{
        cb(ec.E2070);
 ...
 * @param cb
 */
ChannelQueryControl.prototype.handleCQ21 = function(user, headNode, bodeNode, cb){
    var self = this;
    var backBodyNode = {rst:[]};
    var table = dc.main.get("term");
    var cond = {gameCode: bodeNode.gameCode, code:{$in: bodeNode.matchCode}};
    var colunm = {code:'matchCode', gameCode:1, wNum:1, status:1};
    async.waterfall([
        function(cb){
            table.find(cond, colunm, []).sort({code:1}).toArray(function(err, data){
                if(err){
                    cb(err)
                }else{
                    backBodyNode.rst = data;
...
ChannelQueryControl.prototype.handleCQ22 = function(user, headNode, bodyNode, cb)
{
    ...
    async.waterfall([
        function(cb)
        {
            var cursor = table.find(cond, cols, []).sort({code:1});
            cursor.dateToString();
            cursor.toArray(function(err, data){
                //查询的期次为竞猜期次。去查询赔率和明细
                async.eachSeries(data, function (row, callback) {
                    ...
                    if(ticketGame.type == gameType.Jingcai ){
                        ...
                        var fields = {l_cn:1,home_cn:1,guest_cn:1,l_background_color:1};
                        var jiaoyanfields = [];
                        if(bodyNode.pType){
                    ...
                    backBodyNode.count = backBodyNode.rst.length;
                    cb(err, null);
                });
            });
        }
