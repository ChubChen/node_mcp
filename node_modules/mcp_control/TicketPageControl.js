...
TicketPageControl.prototype.list = function(headNode, bodyNode, cb)
{
    ...
    if (add.cashStartDate != null && add.cashStartDate != null && add.cashEndDate != "" && add.cashEndDate != "") {
        add.cashStartDate = dateMathUtil.getEarlyTime(add.cashStartDate);
        add.cashEndDate =  dateMathUtil.getLaterTime(add.cashEndDate);
        backBodyNode.cond.cashTime = {$gte: add.cashStartDate, $lte: add.cashEndDate};
        //页面显示时间格式
        add.cashEndDate = moment(add.cashEndDate).format("YYYY-MM-DD");
        add.cashEndDate =  moment(add.cashEndDate).format("YYYY-MM-DD");
    }
    ...
    async.waterfall([
        ...
        function(cb){
            ...
            var sql = '';
            if(sqlCondition == ''){
                 sql = "select count(*) as num, sum(amount) as amount, sum(bonus) as bonus from tticket  ";
            }else{
                 sql = "select count(*) as num, sum(amount) as amount, sum(bonus) as bonus from tticket where " + sqlCondition;
            }
            log.info(sql);
...
TicketPageControl.prototype.download = function(headNode, bodyNode, cb)
{
    var self = this;
    var backBodyNode = {};
    backBodyNode.sort = {id:-1};
    ...
    var table = dc.main.get("tticket");
    var conf = {rows:new Array()};
    conf.cols = [{caption:"id",type:'number'},{caption:"id",type:'string'},{caption:"出票时间",type:'string'},{caption:"出票状态",type:'string'},
        {caption:"票状态",type:'string'},{caption:"彩种",type:'string'},{caption:"金额",type:'number'},{caption:"中奖金额",type:'number'}];
    var fields = {id:1,printTime:1,printStatus:1,status:1,gameCode:1,amount:1,bonus:1};
    async.waterfall([
        function(cb){
            var cursor = table.find(backBodyNode.cond, {}, []).sort(backBodyNode.sort);
            cursor.dateToString();
            cursor.toArray(function(err, data){
                for(var key in data)
                {
