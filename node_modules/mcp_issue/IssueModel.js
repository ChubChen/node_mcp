                                    ...
                                    if(cacheObj[termCode] == undefined || cacheObj[termCode] == null)
                                    {
                                        JcCache.findOne({"_id":termCode},{},[],function(err, result){
                                            if(result != null && result != undefined){
                                                ...
                                                termTable.findOne({"id":dTerm.gameCode + "_" + termCode},{wNum:1},[],function(err, result){
                                                    if(result.wNum == ''){
                                                        //如果开奖了还没有开奖消息 则拷贝到这期表下面
                                                        table.findAndRemove({}, {}, [], function(err, row){
                                                            var colName = dTerm.gameCode + "_" + termCode + "_" + msg.sep;
                                                            var tempTable = conn.collection(colName);
                                                            tempTable.save(termTable, [],function(){
                                                                callbackMath(null);
                                                            });
                                                        });
                                                    }else{
