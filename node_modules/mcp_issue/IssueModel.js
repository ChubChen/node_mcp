...
IssueModel.prototype.check = function(msg, dTerm, cb)
{
    ...
    async.waterfall([
        ...
        function(check, cb)
        {
            ...
            var JcCache = db.mg.get("jcDrawNumberCache");
            var termTable = db.main.get("term");
            var cacheObj = {};
            if(gameInfoType != gameType.Jingcai ){
                cursor.toArray(function(err, data){
                        async.each(data, function(row, callback){
                            var matchArray = row.number.split(";");
                            for(var i = 0 ; i < matchArray.length ; i++){
                                var termCode = matchArray[i].split("|")[1];
                                if(!cacheObj[termCode]){
                                    jcDrawNumberCache.findOne({"_id":termCode},{},[],function(err, result){
                                        if(!result){
                                          cacheObj[termCode] = {code:termCode, number:result.drawNumber};
                                          check.setDrawNumber({code:termCode, number:result.drawNumber});
                                        }else{
                                            termTable.finOne({"id":row.gameCode + "_" + termCode},{wNum:1},[],function(err, result){
                                                cacheObj[termCode] = {code:termCode, number:result.wNum};
                                                check.setDrawNumber({code:termCode, number:result.wNum});
                                                log.info("从期次表查询期次"+ termCode + "____" + result.wNum);
                                            })
                                        }
                                    });
                                }
                ...
                function (callback) {
                    table.findAndRemove({}, {}, [], function(err, ticket){
                        ...
                        if(ticket)
                        {
                            ...
                            var bonusInfo = check[name]({number:ticket.number, bType:ticket.bType});
