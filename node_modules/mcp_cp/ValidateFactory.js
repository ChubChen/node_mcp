...
var jclqValidate = require("mcp_jclq").validate;
var pl3Validate = require("mcp_pl3").validate;
var pl5Validate = require("mcp_pl5").validate;
var sxcValidate = require("mcp_sxc").validate;
...
ValidateFactory.prototype.validate = function(order, cb)
{
    ...
    order.amount = parseInt(order.amount, 10);
    async.eachSeries(tickets, function(ticket, callback) {
        ticket.amount  = parseInt(ticket.amount, 10);
...
ValidateFactory.prototype.checkTerm = function(terms, ticket, cb)
{
    ...
    if(ticketGame.type == gameType.Jingcai)
    {
        ...
        var termCode = new Array();
        for(var i = 0; i < items.length; i++)
        {
            var curMatch = items[i];
            var curMatchDetailArray = curMatch.split("|");
            termCode.push(curMatchDetailArray[1]);
            keys.push(ticket.gameCode + "_" + curMatchDetailArray[1]);
        }
        if(termCode.length >1){
            ticket.termCode = termCode.sort().pop(); //竞猜表里面存放最后一期的场次
            ticket.oneTermCode = termCode.sort().shift(); //竞猜里面第一期的场次
        }else{
            ticket.termCode = termCode.sort().pop(); //竞猜表里面存放最后一期的场次
            ticket.oneTermCode = ticket.termCode; //竞猜里面第一期的场次
        }
    }
    else
    {
 ...
 * @param cb
 */
ValidateFactory.prototype.validateT52 = function(order, ticket, cb)
{
    var self = this;
    jclqValidate.validate(order, ticket, cb);
}
 ...
 * @param cb
 */
ValidateFactory.prototype.validateT03 = function(order, ticket, cb)
{
    var self = this;
    pl3Validate.validate(order, ticket, cb);
}
 ...
 * @param cb
 */
ValidateFactory.prototype.validateT04 = function(order, ticket, cb)
{
    var self = this;
    pl5Validate.validate(order, ticket, cb);
}
 ...
 * @param cb
 */
ValidateFactory.prototype.validateT02 = function(order, ticket, cb)
{
    var self = this;
    sxcValidate.validate(order, ticket, cb);
}
