        ...
        function (callback) {
            hitTable.findAndRemove({}, {}, [], function(err, ticket){
                ...
                if(ticket)
                {
                    ...
                    strB.append(ticket.auditTermCode).append(split).append(ticket.dNumber).append(split).append(ticketStatus.HIT).append(split).append(ticket.customerId).append(split).append(ticket.bonusStatus);
                    strB.append("\r\n");
                    writeData.write(strB.toString());

                    var sql = 'insert into ' + self.tmpHitTable + '(id,outerId,bonus,bonusDetail,bonusBeforeTax,auditTermCode,dNumber,status,customerId,bonusStatus) values(';
                    sql += ticket._id + ",'" + ticket.outerId + "'," + ticket.bonus + ",'" + bonusDetailStr + "',";
                    sql += ticket.bonusBeforeTax + ",'" + ticket.auditTermCode + "','" + ticket.dNumber + "'," + ticketStatus.HIT + ",'"+ticket.customerId+"',"+ticket.bonusStatus+")";
                    log.info(sql);
                    conn.execute(sql, [], function(err ,data){
                        if(err)
                        {
