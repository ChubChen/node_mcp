...
PrizeJob.prototype._updateHit = function(cb)
{
    ...
    sql += "a.auditTermCode = b.auditTermCode, a.dNumber = b.dNumber, a.status = b.status, a.bonusStatus=b.bonusStatus, a.prizeTime = " + now;
    self._info(sql);
...
PrizeJob.prototype._generateFileByCustomer = function (cb) {
     ...
     async.waterfall([
         ...
         function (cb) {
             var conn = dc.main.getConn();
             var sql = "select concat(customerId,'&',sum(case when bonusStatus=1 then bonus else 0 end),'&',sum(case when bonusStatus=0 then bonus else 0 end)) as 'key' from tticket where termCode = '"+self.dTerm.code+"' and gameCode='"+self.dTerm.gameCode+"' group by customerId";
             log.info(sql);
             conn.execute(sql, [], function (err , data) {
                 if(err){
                     cb(err);
         ...
         function (customerArray, cb) {
             ...
             if(customerArray != undefined && customerArray!= null){
                async.eachSeries(customerArray, function(customer, callback){
                    ...
                    var sql = "select concat(outerId,'^',bonus,'^',bonusBeforeTax,'^',bonusStatus,'^',bonusDetail) as 'record' from " +self.tempTableName+ " where customerId='" +customers[0]+ "'";
                    log.info(sql);
                        ...
                        if(err){
                           ...
                           if(prizeDate){
                               for(var key in prizeDate){
                                  body.record.push(prizeDate[key].record);
                               }
