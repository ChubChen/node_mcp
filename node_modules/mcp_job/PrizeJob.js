...
PrizeJob.prototype._generateFileByCustomer = function (cb) {
     ...
     async.waterfall([
         ...
         function (cb) {
             var conn = dc.main.getConn();
             var sql = "select concat(customerId,'&',sum(case when bonusStatus=1 then bonus else 0 end),'&',sum(case when bonusStatus=0 then bonus else 0 end)) as 'key' from " +self.tempTableName+ " group by customerId";
             log.info(sql);
             conn.execute(sql, [], function (err , data) {
                 if(err){
                     cb(err);
                ...
                async.eachSeries(customerArray, function(customer, callback){
                    ...
                    conn.execute(sql, [], function (err, prizeDate) {
                        if(err){
                           ...
                           var writeData = fs.createWriteStream(path.join(self.baseDir, self.fileName), {
                               flags: 'w',
                               encoding:'utf8'
                           });
                           if(prizeDate){
                               for(var key in prizeDate){
                                  ...
                                  for(var val in sqlDate.bonusDetail){
                                    bonusInfo.push(val.gameGrade.des + '*' + val.count + '注')
                                  }
                                  var tempStr = sqlDate.outerId + splitEsp + sqlDate.bonus + splitEsp + sqlDate.bonusBeforeTax + splitEsp + bonusInfo.join(",") + splitEsp+ prizeDate.bonusStatus + splitEsp + prizeDate.taxStatus;
                     ...
                     client.mkdir(ftpDir, true, function(err){
                         if(err){
                            callback(err);
                             ...
                             log.info("put local" + path.join(self.baseDir, self.fileName)+ "To remote"+ path.join(ftpDir, self.fileName) );
                             client.append(path.join(self.baseDir, self.fileName), path.join(ftpDir, self.fileName), function(){
                                callback(null);
                             });
                         }
                     });
