...
var mcp_config = require("mcp_config");
var prop = mcp_config.prop;
...
PrizeJob.prototype._updateHit = function(cb)
{
    ...
    sql += "a.auditTermCode = b.auditTermCode, a.dNumber = b.dNumber, a.status = b.status, a.bonusStatus=b.bonusStatus, a.taxStatus=b.taxStatus, a.prizeTime = " + now;
    self._info(sql);
...
//生成渠道用户中奖文件
PrizeJob.prototype._generateFileByCustomer = function (cb) {
     var self = this;
     async.waterfall([
         //生成本地渠道用户文件 到/data/app/channelIssue/游戏/年月/日/期次 场次
         //创建目录
         function (cb) {
         ...
         function (cb) {
             ...
             var cond = {gameCode : self.dTerm.gameCode};
             var cols = {customerId:1};
             var cursor = table.find(cond, cols, []);
             cursor.toArray(function(err, datas){
                 cb(err, datas);
             })
         },
         function(cusDatas, cb) {
             var customerArray = [];
             async.eachSeries(cusDatas, function(customer, callback) {
                 var customerId = customer.customerId;
                 var conn = dc.main.getConn();
                 var sql = "select concat(customerId,'&',sum(case when bonusStatus=1 then bonus else 0 end),'&',sum(case when bonusStatus=0 then bonus else 0 end)) as 'key' from " +self.tempTableName+ " where customerId = '"+customerId+"' group by customerId";
                 log.info(sql);
                 conn.execute(sql, [], function (err , data) {
                     if(err){
                         cb(err);
			...
			log.info(data);
                        if(data && data.length !=0 ){
                            customerArray.push(data[0].key);
			    callback(null);
                        }else{
                            customerArray.push(customerId+"&0&0");
                            callback(null);
			}
                     }
                 });
             }, function (err) {
                 cb(err, customerArray);
             });
         },
         //生成文件
         function (customerArray, cb) {
             ...
             if(customerArray != undefined && customerArray!= null){
                async.eachSeries(customerArray, function(customer, callback){
                    ...
                    head.smallAwardSum = customers[2];
                    head.bigAwardSum = customers[1];
                    head.fileCreateTime = dateUtil.getCurTime();
                    var splitEsp = "^";
                    var body = {record:new Array()};
                    var sql = "select outerId,bonus,bonusBeforeTax,bonusDetail,bonusStatus,taxStatus  from " +self.tempTableName+ " where customerId='" +customers[0]+ "'";
                    log.info(sql);
                    conn.execute(sql, [], function (err, prizeDate) {
                        if(err){
                            cb(err)
                        }else{
                           ...
                           if(prizeDate && prizeDate.size != 0){
                               for(var key in prizeDate){
                                  var sqlDate = prizeDate[key];
                                  var bonusDetail = JSON.parse(sqlDate.bonusDetail);
                                  var bonusInfo = new Array();
                                  for(var iterm in bonusDetail){
                                      bonusInfo.push(gameGrades["lv"+bonusDetail[iterm].level].data.des + "*" + bonusDetail[iterm].count + "注");
                                  }
                                  var tempStr = sqlDate.outerId + splitEsp + sqlDate.bonus + splitEsp + sqlDate.bonusBeforeTax + splitEsp + bonusInfo.join(";") + splitEsp+ sqlDate.bonusStatus + splitEsp + sqlDate.taxStatus;
                     ...
                     client.mkdir(ftpDir, true, function(err){
                         if(err){
                            callback(err);
                             ...
                             client.put(path.join(self.baseDir, customerid+"_"+self.fileName), path.join(ftpDir, self.fileName), function(){
                                callback(null);
                             });
                         }
                     });
...
PrizeJob.prototype.run = function(cb)
{
    ...
    async.waterfall([
        function(cb) {
            self._generateFileByCustomer(function(err, data){
                if(err)
                {
                    log.error(err);
