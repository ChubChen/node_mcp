...
PrizeJob.prototype._generateFileByCustomer = function (cb) {
     ...
     async.waterfall([
         ...
         function (cb) {
             var conn = dc.main.getConn();
             var sql = "select concat(customerId,'&',sum(case when bonus>1000000 then bonus else 0 end),'&',sum(case when bonus<1000000 then bonus else 0 end)) as 'key' from tticket where termCode = '"+self.dTerm.code+"' and gameCode='"+self.dTerm.gameCode+"' group by customerId";
             log.info(sql);
             conn.execute(sql, [], function (err , data) {
                 if(err){
                     cb(err);
         ...
         function (customerArray, cb) {
             ...
             if(customerArray != undefined && customerArray!= null){
                async.eachSeries(customerArray, function(customer, callback){
                    ...
                    var sql = "select concat(id,'^',bonus,'^',bonusBeforeTax,'^',case when bonus >1000000 then 2 else 1 end,'^',bonusDetail) as 'record' from " +self.tempTableName+ " where customerId='" +customers[0]+ "'";
                    log.info(sql);
