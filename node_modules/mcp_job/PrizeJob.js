...
PrizeJob.prototype._generateFileByCustomer = function (cb) {
     ...
     async.waterfall([
         ...
         //创建目录
         function (cb) {
              if(fsUtil.createDirSync(baseDir)){
                  cb(null);
              }else{
         ...
         function (cb) {
             var conn = dc.main.getConn();
             var sql = "select concat(customerId,'&',sum(bonus>1000000),'&',sum(bonus<1000000)) from " +self.tmpHitTable+ " group by customerId";
             log.info(sql);
             conn.exec(sql, [], function (err , data) {
                 if(err){
                     cb(err);
                 }else{
                     cb(null, data);
         ...
         function (customerArray, cb) {
             ...
             if(customerArray != undefined && customerArray!= null){
                async.eachSeries(customerArray, function(customer, callback){
                    ...
                    head.termCode = self.dTerm.termCode;
                    head.smallAwardSum = customers[2];
                    head.bigAwardSum = customers[1];
                    head.fileCreateTime = dateUtil.getCurTime();
                    var body = {};
                    var sql = "select concat(id,'^',bonus,'^',bonusBeforeTax,'^',case when bonus >1000000 then 2 else 1 end,'^',bonusDetail) from " +self.tmpHitTable+ " where customerId='" +customer+ "'";
                    conn.exec(sql, [], function (err, prizeDate) {
                        if(err){
                            cb(err)
                        }else{
         ...
         function (customerIdAarr, cb) {
             ...
             client.on('ready', function(){
                 async.eachSeries(customerArray, function(customerid, callback){
