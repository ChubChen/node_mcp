...
LotDetailJob.prototype._writeFile = function(head ,cb){
    ...
    async.waterfall([
        function(cb){
            var conn = dc.main.getConn();
            var sql = "select sum(amount) as succAmount, count(id) as succCount from tticket where "+ cond +" and  status = 1300 and customerid = '" +head.userId+ "'"; //出票成功金额和 数量
            conn.execute(sql, [], function (err , data) {
                if(err){
                    cb(err);
                }else{
        ...
        function(successObj, cb){
            if(successObj){
                var conn = dc.main.getConn();
                var sql = "select concat(outerId,^,pType,^,bType,^,amount,^,multiple,^,number,^,FROM_UNIXTIME((printtime/1000), '%Y-%m-%d %H:%i:%s')) as record from tticket where "+ cond +" and  status = 1300 and customerid = '" +head.userId+ "'"; //出票成功金额和 数量
                conn.execute(sql, [], function(err, data){
                    if(err){
                        log.error(err);
                        cb(err);
                        ...
                        if(data){
                            var xmlJson = {file:{head:head, body: data}};
