...
DbCenter.prototype._initMg = function(cb)
{
    ...
    var printqueen = new Table(db, "printqueen", [
        ...
        new Column(db, "closeTime", "bigint", -1, false, undefined),    //票据的创建时间
    ...
    db.put(cus_config);
    var JcOddsLastUpdateTime = new Table(db, "JcOddsLastUpdateTime", [
        new Column(db, "_id", "varchar", 80, false, "JCUPDATETIME", true, false),
        new Column(db, "date", "varchar", 80, true, undefined),    //最后更新时间
    ]);
    db.put(JcOddsLastUpdateTime);

    var jcodds = new Table(db, "jcodds", [
        new Column(db, "_id", "varchar", 80, false, undefined, true, false), //gameCode_matchcode_ptype
        new Column(db, "matchName","varchar", 256, true, undefined),
        new Column(db, "matchCode", "varchar", 40, false, undefined),//比赛场次
        new Column(db, "gameCode", "varchar", 20, false, undefined),
        new Column(db, "createTime", "date", -1, false, undefined),
        new Column(db, "pType", "varchar", 10, false, undefined),    //玩法
        new Column(db, "oddsCode", "varchar", 80, true, undefined),
        new Column(db, "oddsInfo", "varchar", 256, true, undefined),//1,0|1.2|3.0 胜平负
        new Column(db, "oddsName", "varchar", 80, true, undefined),
        new Column(db, "oddsSingle", "int", 2, true, undefined),
        new Column(db, "version", "int", 11, false, 0)
    ]);
    db.put(jcodds);

    var jcDrawNumberCache = new Table(db, "jcDrawNumberCache", [
        new Column(db, "_id", "varchar", 80, false, undefined, true, false), //termCode
        new Column(db, "drawNumber","varchar", 256, true, undefined),
    ]);
    db.put(jcDrawNumberCache);
...
DbCenter.prototype._initMain = function(cb)
{
    ...
    var tticket = new Table(db, "tticket", [
        ...
        new Column(db, "outerId", "varchar", 40, false, undefined,undefined,undefined,true),    //外部id  联合唯一约束
        new Column(db, "customerId", "varchar", 40, false, undefined,undefined,undefined,true),    //所属用户
        new Column(db, "printId", "varchar", 40, false, undefined),    //出票用户
        new Column(db, "gameCode", "varchar", 20, false, undefined),    //游戏
        new Column(db, "termCode", "varchar", 40, false, undefined),    //期次
        ...
        new Column(db, "bType", "varchar", 10, false, undefined),    //投注方式
        new Column(db, "createTime", "date", -1, false, undefined), //创建时间
        new Column(db, "auditTime", "date", -1, true, undefined), //审计时间
        new Column(db, "printTime", "date", -1, true, undefined), //打印时间
        new Column(db, "cashAmount", "bigint", -1, false, 0),     //税后奖金
        ...
        new Column(db, "bonusDetail", "varchar", 1000, false, ''),     //税后奖金
        new Column(db, "number", "varchar", 1000, false, undefined),    //头注号码
        new Column(db, "dNumber", "varchar", 80, false, ''),    //开奖号码sele
        new Column(db, "rNumber", "varchar", 1000, false, ''),    //竞彩使用
        new Column(db, "maxAwardMoney", "bigint", 1000, false, 0),    //竞彩使用
        new Column(db, "bonusBeforeTax", "bigint", -1, false, 0),     //税前奖金
        new Column(db, "bonusStatus", "int", 11, false, -1), //状态
        new Column(db, "taxStatus", "int", 11, false, -1), //状态
        ...
        new Column(db, "passw", "varchar", 30, false, ''),    //实体票密码
        new Column(db, "terminal", "varchar", 40, false, ''),    //终端机号
        new Column(db, "presetTerminal", "varchar", 40, false, ''),    //终端机号
        new Column(db, "version", "int", 11, false, 0)  //版本
    ]);
    db.put(tticket);
    var hitticket = new Table(db, "hitticket", [
        ...
        new Column(db, "status", "int", 11, false, -1), //状态
        new Column(db, "bonusStatus", "int", 11, false, 0),//大小奖
        new Column(db, "taxStatus", "int", 11, false, 0),//是否扣税
    ...
    var term = new Table(db, "term", [
        ...
        new Column(db, "totalsalemoney", "bigint", 11, false, -1),
        new Column(db, "wNum", "varchar", 80, false, ''),
        new Column(db, "version", "int", 11, false, 0)
    ]);
    db.put(term);
    ...
    db.put(moneylog);
    var errLog = new Table(db, "errLog", [
        new Column(db, "id", "bigint", -1, false, undefined),
        new Column(db, "content", "varchar", 1024, false, undefined), //错误的数据
        new Column(db, "errDesc", "varchar", 80, false, undefined)
    ]);
    db.put(errLog);
    self.main = db;
    self.main.init(cb);
